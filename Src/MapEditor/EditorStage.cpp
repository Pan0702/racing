...
{
    // メッシュ初期化の共通処理
    auto initMesh = [](CFbxMesh*& mesh, MeshCollider*& coll,const char* name)
    {
        CModelStorage* m = ObjectManager::FindGameObject<CModelStorage>();
        mesh = m->GetModel(name);
        coll = m->GetCollider(name);
    };

    initMesh(loadStraightMesh, loadStraightColl,"Plane");
    initMesh(loadCurveMesh, loadCurveColl,"Curve");
    initMesh(loadGoalMesh, loadGoalColl,"GoalLine");
}
